...
export function initSentry() {
  ...
  Sentry.init({
    ...
    release: (env as any).COMMIT,
    tracesSampleRate: 0,
  })

  Sentry.setTag('repo', (env as any).REPO)
  Sentry.setTag('ref', (env as any).REF)
  Sentry.setTag('commit', (env as any).COMMIT)
...
export async function flush(timeoutMs = 2000) {
  if (!enabled || !initialized) return true
