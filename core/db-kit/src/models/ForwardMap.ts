...
export class ForwardMap {
  ...
  find(target: any) {
    ...
    if (typeof target === 'object' && 'uin' in target) return this.findByQQ((target as any).uin)
    if (typeof target === 'object' && 'gid' in target) return this.findByQQ((target as any).gid)
  ...
  async add(qqRoomId: string | number | bigint, tgChatId: string | number | bigint, tgThreadId?: bigint) {
    ...
    if (existingByQQ) {
      if (existingByQQ.tgChatId === BigInt(tgChatId) && (existingByQQ.tgThreadId ?? null) === normalizedThreadId) {
        return existingByQQ
      }
